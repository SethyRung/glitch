...
export function computeBridgeHash(userId: string, name: string, email: string): string {
  const { bridgeSharedSecret: secret } = useRuntimeConfig();
  if (!secret) throw new Error("NUXT_BRIDGE_SHARED_SECRET is not configured");
  return createHash("sha256").update(`${userId}${name}${email}${secret}`).digest("hex");
}

export function verifyBridgeHash(
  userId: string,
  name: string,
  email: string,
  candidate: string,
): boolean {
  const expected = computeBridgeHash(userId, name, email);
