...
version: alpha
name: Glitch
description: A game-store demo for WebBridge — violet primary, Geist type, mono details, and pixel-letter emphasis composed on top of Nuxt UI's zinc neutral surface.
colors:
  primary: "#8B5CF6"
  primarySoft: "#A78BFA"
  primaryDeep: "#6D28D9"
...
but stays readable as a marketing and shopping surface. A single violet
...
- **Brand color** — `text-primary`, `bg-primary`, `ring-primary`,
  ...
  dark mode. With the project's violet scale this resolves to `#8B5CF6`
  light / `#A78BFA` dark. Use opacity utilities (`bg-primary/30`,
...
- **Focus** — Every interactive Nuxt UI component carries a `focus-visible`
  ...
  mirror that pattern, not paint a raw violet ring.
...
- **Primary `#8B5CF6` (Violet 500):** the only saturated hue on the
  ...
  words. Tinted lighter in dark mode (`primary-400: #A78BFA`).
- **Primary Deep `#6D28D9` (Violet 700):** hover and pressed states for
...
variables; the violet stays the violet (just one shade lighter).

**Accessibility note.** `#8B5CF6` itself does not meet WCAG AA (4.5:1)
against white at body sizes — its contrast ratio is ~3.9:1. The catalog
...
and clears AA at 18px+). When you need a solid violet CTA, step down to
`primaryDeep` (`#6D28D9`, ~7.5:1) for the background and keep the label
white.
...
violet — it conflicts with the brand swatch.
...
- Keep one violet accent per viewport. If a heading already uses a pixel
